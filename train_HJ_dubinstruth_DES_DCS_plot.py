...
def plot_hj_ensemble_comparison(ensemble, des_methods, brs_data, checkpoint_epochs, 
                               x_min=-3.0, x_max=3.0, y_min=-3.0, y_max=3.0, nx=100, ny=100):
    ...
    for theta in thetas:
        ...
        for i, (method_name, method) in enumerate(working_methods[:3]):  # Top 3 methods
            if plot_idx < len(axes):
...
def main():
    ...
    results, fitted = evaluate_des_methods(X_train, y_train, X_test, y_test, ensemble)

    # 6. Log DES results
    wandb.log({f"des_accuracy/{k}": v['accuracy'] for k, v in results.items() if v['predictions'] is not None})
    ...
    plot_hj_ensemble_comparison(ensemble, fitted, brs_data, CHECKPOINT_EPOCHS,
