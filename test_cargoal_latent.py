...
class CarGoalEnvForTesting:
    ...
    def reset(self, state=None):
        ...
        return obs, {"input_dreamer":obs_proc}
...
class HJPolicyEvaluator:
    ...
    def encode_observation(self, obs):
        ...
        visual_tensor = self.transform(torch.from_numpy(visual_np))
        visual_tensor = visual_tensor.unsqueeze(0).unsqueeze(0).to(self.device).float()
    ...
    def predict_next_state_value(self, obs, action, return_debug_info=False):
        ...
        with torch.no_grad():
            try:
                ...
                action_tensor = torch.from_numpy(np.stack(action_list)).unsqueeze(0).to(self.device).float()
...
def simulate_cargoal_with_hj(hj_evaluator, env, mode="switching", max_steps=200, 
    ...
    Args:
        ...
        mode: "switching", "dreamer_only", or "safe_only"
    ...
    # Create Dreamer controller
    dreamer_controller = DreamerController()
    ...
    while not done and step_count < max_steps:
        ...
        if mode == "safe_only":
            ...
            if use_dynamics:
                ...
                next_hj_hj = hj_evaluator.get_action_hj_values(obs, action).item()
            
        elif mode == "dreamer_only":
            # Always use Dreamer controller
            action = dreamer_controller.get_action(proprio_state)
            using_hj = False
            dreamer_actions_taken.append(action.copy())
            # Calculate what next HJ would be with this Dreamer action
            if use_dynamics:
                next_hj_hj = hj_evaluator.predict_next_state_value(obs, action)
            else:
                next_hj_hj = hj_evaluator.get_action_hj_values(obs, action).item()
        elif mode == "hard_switch":  # switching mode
            # Get Dreamer action
            dreamer_action = dreamer_controller.get_action(proprio_state)
            
            # Predict HJ value of next state with Dreamer action (with debug info)
            if use_dynamics:            
                if step_count < 10:  # Only get debug info for first 10 steps to avoid slowdown
                    next_hj_value, predicted_img, predicted_prop = hj_evaluator.predict_next_state_value(
                        obs, dreamer_action, return_debug_info=True)
                else:
                    next_hj_hj = hj_evaluator.predict_next_state_value(obs, dreamer_action)
                    predicted_img, predicted_prop = None, None
            else:
                next_hj_hj = hj_evaluator.get_action_hj_values(obs, dreamer_action).item()
                predicted_img, predicted_prop = None, None   
            
            # Switch to safe controller if next state would be unsafe
            if next_hj_hj < safety_threshold:
                action = hj_evaluator.get_safe_action(obs)
                using_hj = True
                hj_interventions += 1
                if last_controller == "Dreamer":
                    total_switches += 1
                print(f"Step {step_count}: HJ intervention! Next HJ would be {next_hj_hj:.3f}")
                print(f"  Dreamer action: {dreamer_action}, HJ action: {action}")
                ...
                if use_dynamics:
                    ...
                    next_hj_hj = hj_evaluator.get_action_hj_values(obs, action).item()
                predicted_hj_values.append(next_hj_hj)
            else:
                action = dreamer_action
                using_hj = False
                if last_controller == "HJ":
                    total_switches += 1
                last_controller = "Dreamer"
                dreamer_actions_taken.append(action.copy())
                next_hj_pid = next_hj_hj
                predicted_hj_values.append(next_hj_hj)  # Store PID prediction for PID action
        else:  # qp switching mode
            # Get Dreamer action
            dreamer_action = dreamer_controller.get_action(proprio_state)
            
            # Predict HJ value of next state with Dreamer action (with debug info)
            if step_count < 10:  # Only get debug info for first 10 steps to avoid slowdown
                next_hj_hj, predicted_img, predicted_prop = hj_evaluator.predict_next_state_value(
                    obs, dreamer_action, return_debug_info=True)
            else:
                next_hj_hj = hj_evaluator.predict_next_state_value(obs, dreamer_action)
                predicted_img, predicted_prop = None, None
            ...
            if next_hj_hj < safety_threshold:
                ...
                if safe_mask.sum() != 0:
                    dreamer_action = torch.tensor(dreamer_action).to(safe_mask.device)
                    actions = actions.to(safe_mask.device)
                    distance = torch.norm(actions[safe_mask] - dreamer_action, dim=-1)
                    idx = distance.argmin()
                    print(idx)
                    action = actions[safe_mask][idx]
                    action = action.cpu().numpy()
                    dreamer_action = dreamer_action.cpu().numpy()
                    flag = "QP"
                else:
                    action = hj_evaluator.get_safe_action(obs)
                    flag = "Actor"
                ...
                print(f"  Dreamer action: {dreamer_action}, HJ {flag} action: {action}")
        ...
        if frame is not None:
            ...
            controller_text = "HJ" if using_hj else "Dreamer"
            ...
            if using_hj and next_hj_hj is not None:
                ...
                cv2.putText(frame_with_info, f"HJ(next) using Dreamer: {next_hj_pid:.2f}", 
                           (10, 45), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
        ...
        if step_count % 50 == 0:
            print(f"Step {step_count}: HJ={current_hj_value:.3f}, Cost={cost:.3f}, "
                  f"Controller={'HJ' if using_hj else 'Dreamer'}")
    ...
    if save_video and frames:
        ...
        video_name = f"cargoal_hj_{mode}_run{run_id}_{timestamp}.mp4"
    ...
    if debug_plot and len(predicted_hj_values) > 0:
        create_debug_plots(predicted_hj_values, actual_hj_values, dreamer_actions_taken, 
...
def create_debug_plots(predicted_hj_values, actual_hj_values, dreamer_actions, hj_actions, 
                      video_path, run_id):
    ...
    # Plot 3: Dreamer actions distribution
    if len(dreamer_actions) > 0:
        dreamer_flat = np.concatenate(dreamer_actions) if dreamer_actions[0].ndim > 0 else np.array(dreamer_actions)
        axes[1, 0].hist(dreamer_flat, bins=20, alpha=0.7, color='blue', edgecolor='black')
        axes[1, 0].set_xlabel('Action Value')
        axes[1, 0].set_ylabel('Frequency')
        axes[1, 0].set_title(f'Dreamer Actions Distribution (n={len(dreamer_actions)})')
        axes[1, 0].grid(True)
    else:
        axes[1, 0].text(0.5, 0.5, 'No Dreamer actions', ha='center', va='center')
...
def main():
    ...
    # Paths
    # wm_ckpt_dir = "/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/output3_frameskip1/cargoal/vc1"
    wm_ckpt_dir = "/storage1/fs1/sibai/Active/ihab/research_new/checkpt_dino/output3_frameskip1/cargoal/dino_cls"
    hj_ckpt_dir = "/storage1/fs1/sibai/Active/ihab/research_new/dino_wm/runs/ddpg_hj_latent_cargoal/dino_cls-0806_1214/epoch_120"
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    video_save_path = f"/storage1/fs1/sibai/Active/ihab/research_new/dino_wm/cargoal_test/{timestamp}"
    ...
    modes = ["switching", "dreamer_only", "safe_only", "hard_switch"]
    num_runs_per_mode = 5
    ...
    for mode in modes:
        ...
        for run_id in range(num_runs_per_mode):
            ...
            env = CarGoalEnvForTesting(device)
