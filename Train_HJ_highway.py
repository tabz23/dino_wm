        ...
        while steps < max_steps:
            ...
            for idx, env_idx in enumerate(active_idxs):
                ...
                if use_switching[env_idx]:
                    ...
                    if next_hj_value < 0 or next_state[5] < 0.3:
                        # Switch to HJ safe action
...
def main():
    ...
    log_dir = Path(f"runs/ddpg_hj_highway/{timestamp}/")
    ...
    for epoch in range(1, args.total_episodes + 1):
        ...
        collector.collect_trajectories(buffer, max_steps=4000)
    ...
    # Test the policy on several random initial states
    # test_states = []
    # for _ in range(8):
    #     test_env = HighwayWrapper(device=device)
    #     test_obs, _ = test_env.reset()
    #     test_states.append(test_obs.copy())
    
    # for idx, test_state in enumerate(test_states):
    #     # Plot environment state
    #     ax_env = plt.subplot(4, 4, 2*idx + 1)
    #     test_env.reset(state=test_state)
    #     rgb_array = test_env.render(mode="rgb_array")
    #     ax_env.imshow(rgb_array)
    #     ax_env.axis('off')
    #     ax_env.set_title(f'Test State {idx+1}')
        
    #     # Plot corresponding HJ
    #     ax_hj = plt.subplot(4, 4, 2*idx + 2)
    #     vals = compute_hj_grid_vectorized(policy, test_env, test_state, args, device)
    ...
    #     ego_x, ego_y = test_state[3], test_state[4]
    #     ax_hj.plot(ego_x, ego_y, 'k*', markersize=15)
        
    #     # Get the HJ value at current position
    #     x_idx = int((ego_x - 0.0) / (2.0 - 0.0) * (args.nx - 1))
    #     y_idx = int((ego_y - 0.0) / (20.0 - 0.0) * (args.ny - 1))
    #     x_idx = np.clip(x_idx, 0, args.nx - 1)
    #     y_idx = np.clip(y_idx, 0, args.ny - 1)
    #     current_hj_value = vals[x_idx, y_idx]
        
    #     ax_hj.set_xlabel("Ego X")
    #     ax_hj.set_ylabel("Ego Y")
    #     ax_hj.set_title(f"HJ={current_hj_value:.3f}")
    #     ax_hj.grid(True, alpha=0.3)
    
    # plt.suptitle(f"Final Policy Evaluation after {args.total_episodes} Episodes", fontsize=16)
    # plt.tight_layout()
